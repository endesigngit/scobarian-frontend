...
export default function HeaderCart({ className, isActiveHeader }: HeaderCartProps) {
  ...
  return (
    <Link
      ...
      className={clsx(
        styles.cart,
        cartProduct.length == 0 && styles.cart__empty,
        className,
        isActiveHeader && styles.cart__active
      )}
    >
      ...
      <span className={styles.cart_count}>{productCount}</span>
    </Link>
  )
}
